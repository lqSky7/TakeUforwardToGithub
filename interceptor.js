(() => {
  ...
  XHR.send = function () {
    this.addEventListener("load", function () {
      try {
        if (
          this.url.includes("/api/onlineJudge/submitCode") &&
          this.method.toLowerCase() === "post"
        ) {
          const response = JSON.parse(this.responseText);
          const submissionData = {
            success: response.failedTestCase ? response.failedTestCase.success : false,
          ...
          if (submissionData.results.length > 0) {
            const avgTime =
              submissionData.results.reduce((sum, r) => sum + r.time, 0) /
              submissionData.results.length;
            const avgMemory =
              submissionData.results.reduce(
                (sum, r) => sum + r.memoryStorage,
                0,
              ) / submissionData.results.length;
